...
public class Principal {
	...
	public static void main(String[] args) {
		...
		daoP.vaciarProductos();
		daoC.vaciarCategorias();
		...
		for (Categoria cat : listcat) {
		    if (daoC.altaCategoria(cat) != 0)
		    	System.out.println("Se agrego la categoria: " + cat.getIdCategoria() + " | " + cat.getNombre());
		    else
		        System.out.println("No se pudo agregar la categoría: " + cat.getIdCategoria() + " | " + cat.getNombre());
		}
		...
		Categoria catMod = new Categoria(2, "Accion", false);
		...
		for (Producto pro : listpro) {
			if (daoP.altaProducto(pro) > 0)
				System.out.println("Se agrego el producto: " + pro.getCodigo() + " | " + pro.getNombre());
			else	
				System.out.println("No se pudo agregar el producto: " + pro.getCodigo() + " | " + pro.getNombre());
		}
		...
		if (daoP.bajaProducto("AG01") != 0) {
			System.out.println("Producto " + listpro.get(0).getNombre() + " dado de baja");
		} else
		...
		Producto proMod = new Producto("AK05", "Shrek 2", 25000, 15, 1, true);
		...
		System.out.println("\n===== ALTA DE PRODUCTOS CON PROCEDIMIENTO ALMACENADO =====\n");
		...
		listproSP.add(new Producto("OL16", "Rapunzel", 31000, 12, 3));
